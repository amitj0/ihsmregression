...
public class MsitErrorLogsCountDaily {

	private static final Logger log = LogManager.getLogger(MsitErrorLogsCountDaily.class);
	...
	@FindBy(xpath = "(//div[@id='divSelectEmployeeRights']//div[contains(@class,'departmentname')])[1]")
	private WebElement selectEmployeeRights;

	@FindBy(xpath = "//div[@id='divSelectEmployeePosition']//div[@class='departmentname']")
	private WebElement choosePositon;
	...
	public void setUp() {
		log.info("========================================");
		log.info("   MSIT ERROR LOGS COUNT DAILY TEST - STARTING ");
		log.info("========================================");
		...
		driver.get("https://intels.msit.university/#/");
	...
	public void verifyErrorLogCount() throws InterruptedException {
		String email = "Emp-001";
		String password = "tech@7id=1";
		...
		String errorCount = searchAndCaptureTodayCount(); // first wait here until value comes or timeout
		String currentDate = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd-MMMM-yyyy hh:mm a"));

		String whatsappMessage = "*MSIT -> Error Count Details:* " + "Error Count: " + errorCount + " | Date: "
				+ currentDate;

		log.info(">> Final MSIT Error Count Details Message: " + whatsappMessage);
	...
	public void navigateToErrorLog() throws InterruptedException {
		wait.until(ExpectedConditions.visibilityOf(selectEmployeeRights));
		blinkElement(selectEmployeeRights);
		safeClick(selectEmployeeRights);
		log.info(">> Employee rights selected");

		wait.until(ExpectedConditions.visibilityOf(choosePositon));
		blinkElement(choosePositon);
		safeClick(choosePositon);
		log.info(">> Position selected");

		driver.get("https://intels.msit.university/#/Errorlogs");
		Thread.sleep(2000);
		log.info(">> Navigated to MSIT Error Logs page");
	}
