...
public class IHSM_ClassSchedulingInfoOfflineLECNotScheduleTest extends BaseClass {

	private Map<String, String> stepStatus = new LinkedHashMap<>();
	private SoftAssert soft = new SoftAssert();

	@Test(enabled = true, groups = "Regression", priority = 1, description = "Verify Class Scheduling (Offline) LEC Not Scheduling Test", dataProvider = "ClassScheduling3", dataProviderClass = ClassSchedulingDataProvider.class)
	public void verifyClassSchedulingPRA(ClassSchedulingData3 data) throws Exception {
		...
		try {
			...
			classInfo.fillClassSchedulingOffline2(data.getAcademicSession(), data.getBatch(), data.getAcademicPlan(),
					...
					data.getEndDate(), data.getDay(), data.getRoom(), data.getFrequency(), data.getTimeSlot());

			String actualMsg = classInfo.errorModalMessage();
			Assert.assertEquals(actualMsg,
					"This Combination is not Possible because Schedule is already available for this Combination. Please click View for more Details.");
			isSuccess = true;
			node.pass("Class Scheduling LEC Not Scheduling added successfully");

		} catch (Exception e) {
			node.fail("Class Scheduling LEC Not Scheduling failed: " + e.getMessage());

			throw new RuntimeException(e);
		} finally {
			stepStatus.put(
					"Class Scheduling LEC Not Scheduling [" + data.getAcademicSession() + "-" + data.getBatch() + "]",
					isSuccess ? "PASS" : "FAIL");
		}
	}
