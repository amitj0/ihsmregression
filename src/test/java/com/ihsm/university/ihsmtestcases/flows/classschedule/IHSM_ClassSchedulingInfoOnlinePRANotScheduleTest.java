...
public class IHSM_ClassSchedulingInfoOnlinePRANotScheduleTest extends BaseClass {

	private Map<String, String> stepStatus = new LinkedHashMap<>();
	private SoftAssert soft = new SoftAssert();

	@Test(enabled = true, groups = "Regression", priority = 0, description = "Verify Class Scheduling (Online) PRA Not Scheduling Test", dataProvider = "ClassScheduling", dataProviderClass = ClassSchedulingDataProvider.class)
	public void verifyClassSchedulingLEC(ClassSchedulingData data) throws Exception {
		...
		try {
			...
			classInfo.fillClassSchedulingInformatio3(data.getAcademicSession(), data.getBatch(), data.getAcademicPlan(),
					...
					data.getEndDate(), data.getDay(), data.getTimeSlot(), data.getFrequency());

			String actualMsg = classInfo.errorModalMsg();
			...
			isSuccess = true;
			node.pass("Class Scheduling PRA Not Scheduling added successfully");

		} catch (Exception e) {
			node.fail("Class Scheduling PRA Not Scheduling failed: " + e.getMessage());
	...
	public void summarizeClassSchedulingFlow() {
		System.out.println("\n==== Class Scheduling (Online) Flow Status ====");
